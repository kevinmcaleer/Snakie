...
describe('canvasPxPerMm — the wiring canvas scale, shared (#716)', () => {
  it('is a FIXED scale — a large body on the canvas never rescales the others', () => {
    ...
    expect(canvasPxPerMm([{ width: 95, height: 25 }, { width: 51, height: 21 }])).toBeCloseTo(3.7)
    expect(canvasPxPerMm([{ width: 300, height: 200 }])).toBeCloseTo(3.7)
  })

  it('is the same scale when nothing declares dimensions', () => {
