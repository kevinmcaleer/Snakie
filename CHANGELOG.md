...
- **3-D models for the 2WD robot kit's electronics.** Four more Standard-library
  parts now ship a `model.stl` (mm) so they appear in the Robot View / build
  workspace at true size instead of as a footprint box: the **Raspberry Pi
  Pico 2 W**, the **MX1508** motor driver, the **4×AA battery pack** and the
  **HC-SR04** ultrasonic ranger (the Yellow TT Motor already had one). The
  HC-SR04 is the CC-BY 3.0 model from the
  [FreeCAD-library](https://github.com/FreeCAD/FreeCAD-library), shipped
  untouched with a `meshOffset` that centres it and rests its pins on `z = 0`.
  The other three are generated by the new `scripts/robot-kit-meshes.mjs`
  from named dimensions (Raspberry Pi's mechanical drawing for the Pico, the
  parts' own photos and seller listings for the rest), the same way the
  Modulino and chassis meshes are, with tests holding each model to its part's
  2-D footprint and to the board-flat, Z-up, underside-on-`z = 0` convention.
